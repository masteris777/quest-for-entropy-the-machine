# THE MACHINE. The re-preparation fold, end to end: a pool of free-running rotors, a
# three-number state, an observer that compares three lengths, records the winner as a
# single integer, and rewrites the state from six fresh rotor readings.
#
# build_state_from_pool() is the fold itself; run_model_two() is the machine's whole life.
#
# This is the clean-pool version: identical to its predecessor except for one functional
# change, the pool frequency builder (build_pool -> build_pool_v2, square-free rates with
# a near-resonance screen). Everything else - the step loop, the recipe read from
# metrics_99.json stageB_classA, the gates, the stages, the verdict ladder and its bars -
# is unchanged.
...
# pool-builder margins (declared before the run, binding)
MARGIN_1 = 0.3
MARGIN_2 = 0.1


# ---------------------------------------------------------------------------
# The pool - the ONLY functional change vs the predecessor version.
...
if __name__ == "__main__":
    ...
    # -----------------------------------------------------------------
    # Verdict - first-match order (ladder declared before the run)
    # -----------------------------------------------------------------
