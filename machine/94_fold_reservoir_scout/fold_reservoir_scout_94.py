# Fold sweet-spot and reservoir-quality scout: sweeps the fold strength g and the
# quality of the number source the fold draws from, and locates the band where the
# counted statistics track the squared law.
...
# The fold's first rotation line is copied verbatim from run_model_one in
# embedded_fold_scout_93_v2.py:
#   z_fold = gamma * phi_j + cos_g * z_perp + sin_g * r_perp
# The second line differs BY DESIGN: it carries only the perpendicular
# combination (no rho*phi_j term) because that information is never re-read by
# the observer and is instead appended, write-once, to a tape - vs the earlier
# version, which returned rho*phi_j + ... into a persistent register r.
# Both lines apply the SAME (cos_g, sin_g)
