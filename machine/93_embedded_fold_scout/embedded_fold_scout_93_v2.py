# Embedded-observer fold scout: the observer is put INSIDE the world and the strength of
# the measurement interaction is swept. Defines the substrate turn U_S, the starting state
# z0, the Born target matrix and the memory (Markov) score the rest of the chain uses.
...
# ---------------------------------------------------------------------------
# Constants: realized-convention theta; z0 read at runtime from the frozen Stage A
# metrics in 92_classical_completion/metrics_92.json, with hardcoded fallbacks recorded
# here so the module still resolves if that file is absent
# ---------------------------------------------------------------------------
TWO_PI = 2 * np.pi
theta = np.array([
...
LAB91_D_B1 = {91: 0.4556, 93: 0.4177}  # the classical anchors measured by the two-time stage
...
def run_model_one(Qf, g, T=T_STEPS, track_j0_samples=False):
    ...
    (restricted to steps whose current outcome is j=0 - a declared simplification)."""
    z = z0.copy()
    r = r0.copy()
